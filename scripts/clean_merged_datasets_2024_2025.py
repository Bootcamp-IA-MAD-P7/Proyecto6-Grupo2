...
def clean(df: pl.DataFrame) -> pl.DataFrame:
    ...
    df = df.with_columns(
        pl.col("MainBranch").str.replace("^ +$", None).str.replace("^$", None)
    )
    ...
    medians = df.group_by("Year", "Country").agg(
        pl.median("comp_val").alias("median_comp")
    )
...
if __name__ == "__main__":
    ...
    df.write_parquet(str(output_dir / "merged_survey_2024_2025.parquet"))

    print("Cleaning...")
    clean_df = clean(df)
    print(f"Cleaned: {clean_df.shape}")
    clean_df.write_parquet(str(output_dir / "merged_survey_2024_2025_clean.parquet"))
